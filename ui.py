...
class UiManager:
    def __init__(self, screen, level):
        ...
        self.pause_btn = Button('Пауза', (WIDTH - 70, 40), width=100, height=40,
                                background_color=TAUPE, font_color=WHITE)

        self.start_game_btn = Button('Начать', SCREEN_CENTER)
        ...
        self.start_level_again_btn = Button('Начать заново', SCREEN_CENTER,
                                             background_color=TAUPE, font_color=GREEN)
        self.lose_level_display = Display(buttons=[self.start_level_again_btn])

        self.finish_btn = Button('Завершить', (WIDTH // 2, HEIGHT // 2 + 2 * BTN_HEIGHT))
        self.start_game_again_btn = Button('Начать заново', SCREEN_CENTER)

        # Замените Label2 на изображение завершения игры
        self.win_image = pygame.image.load('game/images/win.png')  # Загрузка изображения завершения игры
        self.win_image = pygame.transform.scale(self.win_image, (400, 400))  # Настройка размера изображения

        self.win_game_display = Display(buttons=[self.start_game_again_btn, self.finish_btn])

        self.lost_image = pygame.image.load('game/images/lost.png')  # Загрузка изображения завершения игры
        self.lost_image = pygame.transform.scale(self.lost_image, (400, 400))  # Настройка размера изображения
    ...
    def draw_window(self, window):
        # Рисуем фон
        self.screen.blit(self.background_image, (0, 0))  # Рисуем изображение фона на экране
        ...
        # Если это экран победы, рисуем дополнительное изображение победы
        if window == self.win_game_display:
            # Рисуем изображение в центре экрана
            self.screen.blit(self.win_image, (WIDTH // 2 - 200, HEIGHT // 2 - 350))  # Настройка позиции изображения
        if window == self.lose_game_display:
            # Рисуем изображение в центре экрана
            self.screen.blit(self.lost_image, (WIDTH // 2 - 200, HEIGHT // 2 - 350))  # Настройка позиции изображения
